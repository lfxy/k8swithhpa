...
// of the given resource for pods matching the given selector in the given namespace, and the current replica count
func (c *ReplicaCalculator) GetMetricReplicas(currentReplicas int32, targetUtilization float64, metricName string, namespace string, selector labels.Selector, servicename string) (replicaCount int32, utilization float64, timestamp time.Time, err error) {
	var metrics metricsclient.PodMetricsInfo
	if metricName == "custom/qps" {
		glog.V(2).Infof("czq new call GetQpsMetric metricName:%s !!!!!!!!!!!!!!!!!", metricName)
		metrics, timestamp, err = c.metricsClient.GetQpsMetric(metricName, namespace, selector, servicename)
	} else {
		glog.V(2).Infof("czq old GetQpsMetric !!!!!!!!!!!!!!!!!")
