...
type MetricsClient interface {
	...
	GetQpsMetric(metricName string, namespace string, selector labels.Selector, servicename string) (PodMetricsInfo, time.Time, error)
}
...
var qpsBaseUrl string = "http://10.15.136.109:8080"
...
func (h *HeapsterMetricsClient) GetRawMetric(metricName string, namespace string, selector labels.Selector) (PodMetricsInfo, time.Time, error) {
	...
	err = json.Unmarshal([]byte(resultRaw), &metrics)
	if err != nil {
		return nil, time.Time{}, fmt.Errorf("failed to unmarshal heapster response: %v", err)
	}

	glog.V(2).Infof("czq Heapster metrics path: %s", metricPath)
	glog.V(2).Infof("czq Heapster metrics result: %s", string(resultRaw))
...
func (h *HeapsterMetricsClient) GetQpsMetric(metricName string, namespace string, selector labels.Selector, servicename string) (PodMetricsInfo, time.Time, error) {
	glog.V(2).Infof("czq this is in GetQpsMetric: %s", servicename)
	if servicename == "" {
		return nil, time.Time{}, fmt.Errorf("servicename cannot be nil in qps hpa")
	}
	podList, err := h.podsGetter.Pods(namespace).List(api.ListOptions{LabelSelector: selector})
	if err != nil {
		return nil, time.Time{}, fmt.Errorf("failed to get pod list while fetching qps metrics: %v", err)
	}
	...
	resultRaw, err := httpGetNginxQps(servicename)
	if err != nil {
		return nil, time.Time{}, fmt.Errorf("failed to get qps from nginx: %v", err)
	}
	glog.V(2).Infof("czq qps GetQpsMetric metrics result: %s", string(resultRaw))

	var metrics heapster.QpsMetricPoint
	err = json.Unmarshal(resultRaw, &metrics)
	if err != nil {
		return nil, time.Time{}, fmt.Errorf("failed to unmarshal qps response: %v", err)
	}


	res := make(PodMetricsInfo, len(podNames))

	timestamp := metrics.Timestamp
	totalQps := metrics.Value
	if totalQps < 0 {
		return nil, time.Time{}, fmt.Errorf("failed to get qps for qps: %d", totalQps)
	}
	averageQps := float64(totalQps) / float64(len(podNames))
...
func httpGetNginxQps(servicename string) ([]byte, error) {
	url := qpsBaseUrl + "/qps/" + servicename
	glog.V(2).Infof("czq qps metrics result: %s", url)
    resp, err := http.Get(url)
    if err != nil {
		return nil, fmt.Errorf("unable to get qps data from %q: %v", url, err)
    }

    if resp == nil {
        return nil, fmt.Errorf("received empty response from %q", url)
    }
    defer resp.Body.Close()
    resultRaw, err := ioutil.ReadAll(resp.Body)
    if err != nil {
		return nil, fmt.Errorf("failed to get qps service, error body: %v", err)
    }
	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("failed to get qps service, error StatusCode: %q, result:%q", strings.TrimSpace(string(resultRaw)))
    }
