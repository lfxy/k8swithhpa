...
var downscaleForbiddenWindow = 2 * time.Minute
var upscaleForbiddenWindow = 2 * time.Minute
...
func (a *HorizontalController) computeReplicasForCustomMetrics(hpa *autoscaling.HorizontalPodAutoscaler, scale *extensions.Scale,
	...
	for _, customMetricTarget := range targetList.Items {
		...
		replicaCountProposal, utilizationProposal, timestampProposal, err := a.replicaCalc.GetMetricReplicas(currentReplicas, floatTarget, fmt.Sprintf("custom/%s", customMetricTarget.Name), hpa.Namespace, selector, customMetricTarget.ServiceName)
		if err != nil {
			lastScaleTime := getLastScaleTime(hpa)
			if time.Now().After(lastScaleTime.Add(upscaleForbiddenWindow)) {
		...
		glog.V(2).Infof("czq qps in computeReplicasForCustomMetrics replicaCountProposal: %d", replicaCountProposal)
...
func (a *HorizontalController) reconcileAutoscaler(hpa *autoscaling.HorizontalPodAutoscaler) error {
	...
	if scale.Spec.Replicas == 0 {
		...
		glog.V(2).Infof("czq in reconcileAutoscaler cpuDesiredReplicas: %d, cmDesiredReplicas:%d, currentReplicas:%d", cpuDesiredReplicas, cmDesiredReplicas, currentReplicas)
		rescaleMetric := ""
		...
		glog.V(2).Infof("czq in reconcileAutoscaler desiredReplicas: %d, currentReplicas:%d", desiredReplicas, currentReplicas)
		rescale = shouldScale(hpa, currentReplicas, desiredReplicas, timestamp)
	}

	glog.V(2).Infof("czq in reconcileAutoscaler rescale:%t", rescale)
	if rescale {
...
func shouldScale(hpa *autoscaling.HorizontalPodAutoscaler, currentReplicas, desiredReplicas int32, timestamp time.Time) bool {
	...
	glog.V(2).Infof("czq in shouldScale desiredReplicas:%d, currentReplicas:%d, LastScaleTime:%v, timestamp:%v", desiredReplicas, currentReplicas, hpa.Status.LastScaleTime, timestamp)
